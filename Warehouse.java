...
public abstract class Warehouse implements Serializable
{
    ...
    public static String doDisplay(String displayBikePart) {
        ...
        while (j < Inventory.toString().size() && !Found) {

            if (Inventory.toString().get(j).getPartName().equals(partName)) {
                Found = true;
                if (Inventory.toString().get(j).getOnSale()) {
                    //Part Name is on sale this is the sales price
                    return (Inventory.toString().get(j).getPartName() + " " + Inventory.toString().get(j).getSalesPrice());
                } else if (!Inventory.toString().get(j).getOnSale()) {
                    //Part Name is not on sale this is the list price
                    return(Inventory.toString().get(j).getPartName() + " " + Inventory.toString().get(j).getListPrice());
                }
            }
            j += 1;
        }
